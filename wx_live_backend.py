...
# ---- WX API stub added by script on 11/20/2025 16:58:00 ----
from typing import List, Optional
from pydantic import BaseModel


class WxReportRow(BaseModel):
    county: str
    state: str
    severity: int
    expectedGust: float
...
@app.get("/api/wx", response_model=List[WxReportRow])
async def api_wx(
    mode: str,
    hours: int = 6,
    sample: int = 50,
    ...
    Matches calls like:
    /api/wx?mode=State&state=Rhode+Island&hours=6&sample=50
    """
    demo_row = WxReportRow(
        county=county or "Demo County",
        state=state or region or "Demo State",
        severity=2,
        expectedGust=35.0,
        expectedSustained=20.0,
        maxGust=45.0,
        maxSustained=30.0,
        probability=0.75,
        crews=3,
