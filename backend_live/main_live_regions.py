...
async def live_wind(lat: float, lon: float, hours: int) -> Tuple[float,float,float,float,float,str]:
  """Best-effort live fetch; never raises; returns (eg, es, mg, ms, p, stamp)."""
  try:
    params = {
      "latitude": lat,
      "longitude": lon,
...
def mk_row(c,s,eg,es,mg,ms,p,pop,stamp):
  ...
  return {
    ...
    "generatedAt": now_iso(), "source": "open-meteo", "upstreamStamp": stamp
  }
...
def indices_for(mode: str, region: str, state: str, sample: int) -> List[int]:
  ...
  # Reduce to top-by-pop (deterministic), then sample if requested
  if not idx:
    return []
...
async def handle(mode: str, region: str, state: str, hours: int, sample: int, nocache: int):
  ...
  sample = max(1, min(1000, sample or 25))  # defaults: 25 rows nationwide/regional, state can override via sample
...
# Primary, alias, and catch-all routes — identical behavior
@app.get("/api/wx")
async def api_wx(req: Request,
                 mode: str = "Nationwide",
                 region: str = "",
                 state: str = "",
                 hours: int = 24,
...
@app.get("/wx")
async def wx_alias(req: Request,
                   mode: str = "Nationwide",
                   region: str = "",
                   state: str = "",
                   hours: int = 24,
