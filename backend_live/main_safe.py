...
# At least a few solid counties per region to guarantee rows for any scope.
...
async def live_wind(lat: float, lon: float, hours: int) -> Tuple[float,float,float,float,float]:
...
async def compute(indices: List[int], hours: int) -> List[Dict]:
  ...
  async def one(i:int):
    try:
      ...
      # Skip any failing item without stopping the response
      pass
...
def cache_key(mode: str, region: str, state: str, hours: int) -> str:
...
async def wx_core(mode: str, region: str, state: str, hours: int):
  try:
    ...
    if not idx: return []
    key = cache_key(mode or "Nationwide", region or "", state or "", h)
    now = time.time()
    hit = CACHE.get(key)
    ...
    # Absolute last resort: never 500 — just return empty
    return []

# Compat routes: /api/wx, /wx, and / all respond with the same behavior
@app.get("/api/wx")
