...
Write-Host "=== FIX WX BACKEND TOP-50 START ===" -ForegroundColor Cyan
...
# Backup current file
$backup = "$backendFile.bak_TOP50_$(Get-Date -Format yyyyMMdd_HHmmss)"
Copy-Item $backendFile $backup -Force
Write-Host "Backup created: $backup" -ForegroundColor Green

# Overwrite backend with optimized top-50 version
@'
import asyncio
...
if __name__ == "__main__":
    ...
    asyncio.run(_demo())
'@ | Set-Content $backendFile -NoNewline

Write-Host "wx_live_backend.py overwritten with TOP-50 optimized version." -ForegroundColor Cyan
Write-Host "=== FIX WX BACKEND TOP-50 DONE ===" -ForegroundColor Cyan
